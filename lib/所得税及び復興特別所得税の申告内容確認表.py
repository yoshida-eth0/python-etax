...
class 所得税_税額控除:
    ...
    TODO
    """
    def __init__(self, 所得税_所得控除: 所得税_所得控除, data: 税額控除の算出に用いるデータ):
...
class 所得税及び復興特別所得税の申告内容確認表_第一表:
    ...
    所得税及び復興特別所得税の申告内容確認表 第一表
    """
    def __init__(self, 所得金額等: 所得金額等, 所得控除データ: 所得控除の算出に用いるデータ, 税額控除データ: 税額控除の算出に用いるデータ):
        # ア~サ. 収入金額等
        # 1~12. 所得金額等
        self.所得金額等 = 所得金額等

        # 13~29. 所得から差し引かれる金額
        self.所得税_所得控除 = 所得税_所得控除(所得金額等, 所得控除データ)

        # 32~52. 税金の計算
        self.所得税_税額控除 = 所得税_税額控除(self.所得税_所得控除, 税額控除データ)
