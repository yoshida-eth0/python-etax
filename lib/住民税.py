...
class 住民税_所得控除:
    ...
    対象:
        ...
        基礎控除
    """
    def __init__(self, 所得金額等: 所得金額等, data: 所得控除の算出に用いるデータ):
        self.所得金額等 = 所得金額等
        self.data = data
...
class 住民税の税額:
    ...
    (4) 合計税額 -> 税額
    """
    def __init__(self, 課税標準額: 課税標準額, 所得税_所得控除: 所得税_所得控除, 住民税_所得控除: 住民税_所得控除, 住民税の税率: 住民税の税率, 合計所得割: float, 税額控除データ: 税額控除の算出に用いるデータ):
        # 課税標準額
        self.課税標準額 = 課税標準額
    ...
    def 調整控除(self):
        ...
        参考:
            ...
            https://www.city.tokyo-nakano.lg.jp/kurashi/zeikin/jyuminzei-kazei/jyuminzei-keisanrei.html
        """
        if self.課税標準額.総所得<=25_000_000:
            # 合計所得金額が2,500万円以下の場合のみ適用
...
class 住民税:
    ...
    def __init__(self, 所得税_所得控除: 所得税_所得控除, 所得控除データ: 所得控除の算出に用いるデータ, 税額控除データ: 税額控除の算出に用いるデータ, 住民税の税率一覧: list[住民税の税率] = [特別区民税, 都民税]):
        ...
        # (2) 所得控除の内訳
        self.住民税_所得控除 = 住民税_所得控除(self.所得金額等, 所得控除データ)
        ...
        self.住民税の税額一覧 = [住民税の税額(self.課税標準額, 所得税_所得控除, self.住民税_所得控除, 住民税の税率, 合計所得割, 税額控除データ) for 住民税の税率 in 住民税の税率一覧]
